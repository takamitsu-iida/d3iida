...
(function() {
  d3iida.mapChart = function module() {
    ...
    function toggleBrush() {
      ...
      svg.selectAll('.brushLayer').data(['dummy']).enter()
        ...
        .attr('debug', function() {
          console.log('brushLayer created');
        })
  ...
  d3iida.mapChart.example = function() {
    ...
    // mapChartをインスタンス化する
    var mapChart = d3iida.mapChart().width(600).height(600);

    // ブラシイベントを拾って、境界に収まる拠点の配列を得る
    mapChart.on('brushing', function(extents) {
      ...
      // 境界ボックスに収まる全レコード返す
      var filtered = siteLocation.top(Infinity);

      console.log(filtered);
    });

    // コンテナにデータを紐付けてcall()する
    container.datum(prefGovLoc).call(mapChart);
   //
  };
  //
})();
...
d3iida.radioButton = function module() {
  ...
  // 描画領域のマージン
  var margin = {top: 5, right: 20, bottom: 0, left: 40};
